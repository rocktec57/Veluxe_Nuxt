...
<template>
  <div class="bg-[#0b131a] pt-8 px-20 flex justify-between items-end" :class="{'pb-6': !openFilters || isHiding}">
    <div class="w-[140px]">
      <button
        @click="toggleOpenFilters()"
        >
        <svg :class="{'rotate-[90deg]': openFilters && !isHiding}" class="transition mr-6 duration-[1s] ease-in-out" xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 24 24"><path fill="none" stroke="white" stroke-linecap="round" stroke-miterlimit="10" stroke-width="1.5" d="M21.25 12H8.895m-4.361 0H2.75m18.5 6.607h-5.748m-4.361 0H2.75m18.5-13.214h-3.105m-4.361 0H2.75m13.214 2.18a2.18 2.18 0 1 0 0-4.36a2.18 2.18 0 0 0 0 4.36Zm-9.25 6.607a2.18 2.18 0 1 0 0-4.36a2.18 2.18 0 0 0 0 4.36Zm6.607 6.608a2.18 2.18 0 1 0 0-4.361a2.18 2.18 0 0 0 0 4.36Z"/></svg>
      </button>
    </div>
    <div 
      v-if="openFilters"
  ...
  <div 
    class="w-full bg-[#0b131a] flex justify-center items-center px-20"
  >
    <div 
    v-if="openFilters"
    :class="{
    ...
    class="w-full bg-[#0b131a] transition-all duration-[1.5s] ease-in-out"
    >
      <div class="w-full flex justify-center items-center">
        <div class="mt-6 bg-[#22900c] opacity-[0.3] h-[1px] w-full py-[1px]"></div>
      </div>
  ...
  <div v-else class="bg-[#0b131a] w-full pb-20 px-20 grid grid-cols-3 gap-10" :class="{'pt-6': openFilters}" :style="{ minHeight: 'calc(100vh - 198px)' }">
    ...
    >
      <div class="w-full flex justify-center items-center rounded-t-xl bg-[#0b131a] text-white test2">
          <p class="py-3 text-2xl font-black italic flex gap-2">
            {{ carro.preco + " €" }}
          </p>
      </div>
      <Swiper
      ...
      <div class="flex flex-col justify-center items-center w-full pl-3 py-3">
        ...
        <div class="w-full flex items-center justify-start text-white mt-6">
          <div class="w-full flex items-center gap-1 justify-start">
            ...
            <p class="text-sm font-semibold">{{ carro.anoReg }}</p>
          </div>
          <div class="w-full flex items-center gap-1 justify-start">
          ...
          <div class="w-full flex items-center gap-1 justify-start">
            ...
            <p class="text-sm font-semibold">{{ carro.kms + " km" }}</p>
          </div>
        </div>
        <NuxtLink  
        :to="localCode=='pt' ? `/stockSingle/${carro.id}` : '/'+localCode+`/stockSingle/${carro.id}`"
...
<style scoped>
.test {
  box-shadow: 0 -1px 5px #22900c, /* Glow above */
              0 1px 5px #22900c,  /* Glow below */
              0 0 5px #22900c,   /* Larger glow around */
              0 0 10px #1abe0f;   /* Intense glowing effect */
}

.test2 {
  box-shadow: 
              0 0 10px #7fc571;   /* Intense glowing effect */
}

.test3 {
  box-shadow: 0 0 10px #7fc571;   /* Intense glowing effect */
}
